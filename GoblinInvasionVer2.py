...
def pauseMenu():
    ...
    text = font.render("Heals left:" + str(healLimit - healCount), 1, (255, 255, 255))
    win.blit(text, (710, 25))
    if (__DEBUG_MODE__):
        print("Pause Blitted ")
...
def countdown(t):
    ...
    HealButton.color = (255, 50, 50)


def init():
    global timer, lives, goblinHealth, hits, score, pause, amo, bullets, bullets1, first, second, run, points, healCount
...
HealSound = pygame.mixer.Sound(resource_path('Game\Heal.wav'))
...
HealButton = button((255, 50, 50), 870, 15, 100, 50, "Heal")
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if pause:
                    if HealButton.isOver(pos):
                        if healCount <= healLimit:
                            if lives < 5:
                                lives = random.randrange((lives + 1), 7)
                            elif lives > 5:
                                try:
                                    lives = random.randrange((lives + 1), 10)
                                except:
                                    pass
