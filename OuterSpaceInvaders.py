...
def redrawGameWindow():
    ...
    if man.alive:
        ...
        text = font1.render('Outer Space Invaders!', 1, (255, 255, 0))
        win.blit(text, ((W - 720) - 25, 100))
    if hits == 0:
        ...
        if goblinHealth - (goblinHealth / 10) > points >= goblinHealth - (goblinHealth / 2):
            ...
            win.blit(text, (380, 280))
        text = font.render('Thank You For Playing Outer Space Invaders', 1, (255, 255, 255))
        win.blit(text, (300, 420))
        text = font.render('You Defeated the Invader in ' + str(timeIs) + ' Seconds!', 1, (255, 255, 255))
    ...
    if man.alive:
        ...
        if goblin.dead:
            goblin.visible = False
            text2 = font.render('Invader Defeated!', 1, (255, 255, 0))
...
pygame.display.set_caption("Outer Space Invaders")
pygame.mixer.init()
...
bg = pygame.transform.scale(pygame.image.load(resource_path('Game/bg2.jpg')), (W, (H + 20)))
MainScreen = pygame.transform.scale(pygame.image.load(resource_path('Game\MainMenu3.png')), (W, (H + 20)))
        ...
        if not powerUP:
            if self.alive:
                ...
                if not self.standing:
                    if self.left:
                        window.blit(pygame.transform.scale(self.walkLeft[self.walkCount // 6], (80, 80)), (self.x, self.y))
                        self.walkCount += 1
                    elif self.right:
                        window.blit(pygame.transform.scale(self.walkRight[self.walkCount // 6], (80, 80)), (self.x, self.y))
                        self.walkCount += 1
                else:
                    if self.left:
    ...
    def draw(self, window):
        ...
        if self.visible:
            if not self.pause:
                ...
                if self.vel > 0:
                    window.blit(pygame.transform.scale(self.walkRight[self.walkCount // 4], (80, 80)), (self.x, self.y))
                    self.walkCount += 1
                elif self.vel < 0:
                    window.blit(pygame.transform.scale(self.walkLeft[self.walkCount // 4], (80, 80)), (self.x, self.y))
                    self.walkCount += 1
...
man = player((W - 300), (H - 135), 64, 64)
goblin = enemy(10, (H - 135), 64, 64, (W - 50), goblinHealth)
...
while run:
        ...
        if hits != 0:
            ...
            if len(bullets1) < amo:
                if not pause and hits != 0 and lives != 0:
                    bulletSound.play()
                    bullets.append(projectile(round(man.x + man.width // 2), round(man.y + man.height // 2), 6, (128, 0, 128), facing))
                    bullets1.append(projectile(round(man.x + man.width // 2), round(man.y + man.height // 2), 6, (128, 0, 128), facing))
    ...
    else: # not alive, not , first time window draw after game # over
